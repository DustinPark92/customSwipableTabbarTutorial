...
class ViewController: UIViewController {
    ...
    func setupTabs() {
        ...
        tabsView.backgroundColor = .blue
        tabsView.anchor(top:view.safeAreaLayoutGuide.topAnchor,left: view.leftAnchor,right: view.rightAnchor,height: 55)
        // Add Tabs (Set 'icon'to nil if you don't want to have icons)
        tabsView.tabs = [
            Tab(icon: UIImage(named: "music"), title: "매수"),
            Tab(icon: UIImage(named: "movies"), title: "매도"),
            Tab(icon: UIImage(named: "movies"), title: "주문"),
            Tab(icon: UIImage(named: "books"), title: "내역")
        ]
    ...
    func setupPageViewController() {
        // PageViewController
        self.pageController = TabsPageViewController()
...
//            contentVC.name = tabsView.tabs[index].title
            ...
            let contentVC = SellVC()
//            contentVC.name = tabsView.tabs[index].title
            contentVC.pageIndex = index
            return contentVC
        } else if index == 2 {
            let contentVC = OrderVC()
            //contentVC.name = tabsView.tabs[index].title
            contentVC.pageIndex = index
            return contentVC
        } else {
            let contentVC = HistoryVC()
            //contentVC.name = tabsView.tabs[index].title
            contentVC.pageIndex = index
            return contentVC
        }
...
extension ViewController: UIPageViewControllerDataSource, UIPageViewControllerDelegate {
    ...
    func getVCPageIndex(_ viewController: UIViewController?) -> Int {
        ...
        case is BuyVC:
            let vc = viewController as! BuyVC
            return vc.pageIndex
        case is SellVC:
            let vc = viewController as! SellVC
            return vc.pageIndex
        case is OrderVC:
            let vc = viewController as! OrderVC
            return vc.pageIndex
        case is HistoryVC:
            let vc = viewController as! HistoryVC
            return vc.pageIndex
        default:
            let vc = viewController as! BuyVC
            return vc.pageIndex
